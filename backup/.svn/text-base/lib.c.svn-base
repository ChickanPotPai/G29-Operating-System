...
void scrollscreen() {
	...
	for (y = 0; y < NUM_ROWS - 1; y++) {
		for (x = 0; x < NUM_COLS; x++) {
			*(char*)(video_mem + ((NUM_COLS * y + x) << 1)) = *(char*)(video_mem + ((NUM_COLS * (y + 1) + x) << 1));
		}
	}

	for (x = 0; x < NUM_COLS; x++) {
		*(char*)(video_mem + ((NUM_COLS * (NUM_ROWS - 1) + x) << 1)) = 0x00;
	}
}
...
void move_cursor_to(int x, int y) {
	...
	set_attribute(cur_position.x_coord, cur_position.y_coord, ATTRIB);
