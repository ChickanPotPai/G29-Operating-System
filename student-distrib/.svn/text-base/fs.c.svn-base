...
int32_t fs_read(uint32_t fd, uint8_t* buf, int32_t nbytes) {
	...
	// if no inode ptr, it must be a directory
	...
	// read data, return bytes read, and increment file position
...
int32_t read_dir(fd_t* fd, uint8_t* buf) {
	...
	// copy each filename into the buffer
...
int32_t read_data (uint32_t inode, uint32_t offset, uint8_t* buf, uint32_t length)
{
	...
	// get pointer to inodes
	inodes = (inode_t*) (boot);
	inode_ptr = &(inodes[inode + 1]);

	length_left = inode_ptr->length - offset;

	// check if trying to read past file
	if (length_left <= 0)
		return 0;

	// check if file is too big for block storage
	...
	// check if trying to read past file, and cut off
	if ((offset + length) > file_length) {
		length = file_length;
	}

	// calculate blocks that will be spanned by the read
	...
	// check if offset will skip over blocks, and how many
	while (offset > BLOCK)
	{
		i++;
		block_count--;
	...
	// read file block by block, then the bytes remaining
	while (bytes_to_read > 0)
	{
		// copy blocks, with the first taking into account the offset
		if (block_count > 0)
		{
		...
		// copy the remainig bytes of the last block
		else
		{
			off_ptr = (uint8_t*) &data[inode_ptr->blocks[i]] + offset;
...
int32_t read_dentry_by_name (const uint8_t* fname, dentry_t* dentry) {
	...
	// cycle through each entry, checking if the name is the same, if so then copy into pointer
...
int32_t read_dentry_by_index (uint32_t index, dentry_t* dentry)
{
	...
	// duplicate dentry of given index into given pointer
